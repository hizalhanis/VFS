...
$('select.v-country').live('change',function(){
	var country = $(this).val();
	var citySelect = $(this).parents('form').find('select.v-city');
	var city = $(this).attr('state');
	var base_url = 'http://halagel.com.my/cart/';
	$(citySelect).html('<option>Loading...</option>');
		$.ajax({
			url: 'system/get_city_list',
			type: 'POST',
	...
	<div class="lcms-plexcart-breadcrumb">
	    <h3><a href="<?php echo $current_page; ?>">Store</a> &raquo; My Account</h3>	
	</div>
	<form class="lcms-plexcart" method="post" action="<?php echo $current_page; ?>account_update_do">
	    ...
	    <table class="form-grid lcms-plexcart-ci">
	    	<tr>
	    		...
	    		<td class="input"><input class="text password" type="password" name="old_password" /></td>
	    	</tr>
	    	<tr>
	    ...
	    <table class="form-grid lcms-plexcart-ci">
	    	...
	    	<tr>
	    		...
	    		<td class="label">State/Province<span class="red">*</span></td>
				<td class="input"><?php echo form_dropdown('state',$this->controls->state_list('Malaysia'), $user->state, 'class="v-city v-dropdown state mandatory"'); ?></td>
	    	</tr>
	    	<tr>
				...
				<td class="input"><?php echo form_dropdown('country',$this->controls->country_list(), $user->country,'class="v-country v-dropdown mandatory country"'); ?></td>		
	    	</tr>
	    </table>
	    <p class="align-right"><input type="submit" class="lcms-btn submit" value="Update" /></p>
	</form>
