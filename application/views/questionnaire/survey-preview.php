		...
		$(document).ready(function(){
			...
			$('div.lcms-survey-preview-slide').each(function(){
				...
				if (cumulativeHeight > 850){
					x++;
					var td = $(titleDiv).clone();
					$(td).find('span').text('Page ' + x);
					$(this).before(td);
...
<body>
	<div class="lcms-survey-page">
		<div class="title">
			...
			<span style="height: 38px; position: absolute; margin-top: 948px; font-size: 7pt; color: black; @media screen { display:none }; @media print { display: block }"class="page">Page 1</span>

			<?php echo $survey->name; ?>

		</div>
		<?php $x = 0; foreach ($questions as $question):  $answers = json_decode($question->answers); ?>
				
				
				<?php if ($question->type == 'section'): ?>
				<div no="<?php echo $x; ?>" class="lcms-survey-preview-slide" style="padding: 0; margin: 0;">
					<?php if ($question->question): ?>
						<h4><?php echo $question->question; ?></h4>
					<?php endif; ?>
					<?php if ($question->comments_description): ?>
						<div><?php echo nl2br($question->comments_description); ?></div>
					<?php endif; ?>
				</div>
