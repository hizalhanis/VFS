...
$.TextboxList = function(element, _options){
	...
	var updateVal = function(){
		...
		if (isJSON(tempVal)){
			eval("var actualVal = " + tempVal);
			if (isArray(actualVal)){
				for (var i = 0; i < actualVal.length; i++){
					add(actualVal[i].plain, actualVal[i].id, actualVal[i].html);	
				}			
			} else {
				add(actualVal.plain, actualVal.id, actualVal.html);
			}
		} else {
			if (tempVal){
				add(tempVal, tempVal, tempVal);
			}
		} 
