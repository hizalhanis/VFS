...
$(document).ready(function(){
		...
		$.ajax({
			...
			success: function (html){
				if (html){
					...
					$('select.v-city').show();
					$('input.state_alt').hide();

				} else {
					$('select.v-city').hide();
					$('input.state_alt').show();
				}
			}
		});
	});
	...
	<div class="lcms-plexcart-breadcrumb">
		<h3><a href="<?php echo $current_page; ?>">Store</a> &raquo; Register as Member</h3>
	</div>
	    ...
	    <form class="lcms-plexcart" method="post" action="<?php echo $current_page; ?>register_do">
	    	...
	    	<table class="form-grid lcms-plexcart-ci">
	    		<tr>
	    			...
	    			<td class="input"><?php echo form_dropdown('country',$this->controls->country_list(), 'Malaysia','class="v-country v-dropdown mandatory country"'); ?></td>
	    			<td colspan="2"></td>
	    		</tr>					
	    		<tr>
	    		...
	    		<tr>
	    			...
	    			<td class="input"><?php echo form_dropdown('state',$this->controls->state_list('Malaysia'), '', 'class="v-city v-dropdown"'); ?><input class="text state_alt" style="display:none" type="text" name="state_alt" value=""/></td>
	    			<td colspan="2"></td>
	    		</tr>
	    		<tr>	
