...
class CI_Upload {
	...
	function do_upload($field = 'userfile', $index = 0)
	{
		...
		if ( ! is_uploaded_file($_FILES[$field]['tmp_name']) && ! is_uploaded_file($_FILES[$field]['tmp_name'][$index]))
		{
		...
		echo $_FILES[$field]['name'][$index] . '!-';
		

		// Set the uploaded data as class variables
		$this->file_temp = $file_array ? $_FILES[$field]['tmp_name'][$index] : $_FILES[$field]['tmp_name'];		
		$this->file_name = $file_array ? $_FILES[$field]['name'][$index] : $_FILES[$field]['name'];
		$this->file_name = $file_array ? $this->_prep_filename($_FILES[$field]['name'][$index]) : $this->_prep_filename($_FILES[$field]['name']);
		$this->file_size = $file_array ? $_FILES[$field]['size'][$index] : $_FILES[$field]['size'];		
		$this->file_type = $file_array ? preg_replace("/^(.+?);.*$/", "\\1", $_FILES[$field]['type'][$index]) : preg_replace("/^(.+?);.*$/", "\\1", $_FILES[$field]['type']);
		$this->file_type = strtolower($this->file_type);
		$this->file_ext	 = $file_array ? $this->get_extension($_FILES[$field]['name'][$index]) : $this->get_extension($_FILES[$field]['name']);
