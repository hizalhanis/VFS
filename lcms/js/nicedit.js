...
var nicEditorInstance = bkClass.extend({
	...
	construct : function(e,options,nicEditor) {
		...
		if(isTextarea || this.options.hasPanel) {
			...
			var s = {width: '100%', border : '0px solid #ccc', overflowY : 'auto', overflowX: 'hidden' };
			s[(ie7s) ? 'height' : 'maxHeight'] = (this.ne.options.maxHeight) ? this.ne.options.maxHeight+'px' : null;
			this.editorContain = new bkElement('DIV').setStyle(s).appendBefore(e);
			var editorElm = new bkElement('DIV').setStyle({width : '100%', margin: '0px', minHeight : newY+'px'}).addClass('main').appendTo(this.editorContain);
...
var nicEditorPanel = bkClass.extend({
	construct : function(e,options,nicEditor) {
		...
		this.panelContain = new bkElement('DIV').setStyle({overflow : 'hidden', width : '100%', border : '0px solid #cccccc', backgroundColor : '#efefef'}).addClass('panelContain');
			...
			if(type == 'title') {
				...
				switch(type) {
					case 'text':
						...
						break;
					case 'browse':
						this.inputs[itm] = new bkElement('div').setAttributes({id : itm, 'value' : val, 'type' : 'text'}).setStyle({}).appendTo(contain);
						$('#'+itm).html('<div style="height: 100px;" class="lcms-file-image-container"></div><select class="lcms-fic-folder-list" style="width: 100%"><option value=".">All</option></select><ul style="height: 150px" class="lcms-file-list lcms-richtext-file-list"></ul>');
						
						$.ajax({
							url: base_url + 'file/files/Images',
							success: function (html){
								$('ul.lcms-richtext-file-list').html(html);
								
								var addedFolders = [];
								$('select.lcms-fic-folder-list').html('');
								$('select.lcms-fic-folder-list').append('<option value=".">All</option>');
								$('ul.lcms-richtext-file-list li').each(function(){
								    var folder = $(this).attr('folder');
								    if (!addedFolders[folder] && folder != ''){
								    	$('select.lcms-fic-folder-list').append('<option value="'+folder+'">'+folder+'</option>');
								    	addedFolders[folder] = true;
								    }
								
								});
								
								$('select.lcms-fic-folder-list').change(function(){
								    var folder = $(this).val();
								    
								    $('ul.lcms-richtext-file-list li').removeClass('lcms-current-folder');
								    $(this).addClass('lcms-current-folder');
								    
								    if (folder == '.'){
								    	$('ul.lcms-richtext-file-list li').show();
								    } else {
								    	$('ul.lcms-richtext-file-list li').hide();
								    	$('ul.lcms-richtext-file-list li[folder='+folder+']').show();
								    }
								});
								
								$('ul.lcms-richtext-file-list li').click(function(){
									var imgname = $(this).text();
									$('ul.lcms-richtext-file-list li').removeClass('current');
									$(this).addClass('current');
									$('div.lcms-file-image-container').html('<img style="max-height: inherit;" src="'+base_url+'media/'+imgname+'" />');
									$('#src').val(base_url+'media/'+imgname);
								});
						...
						break;
					case 'page_list':
						this.inputs[itm] = new bkElement('div').setAttributes({id : 'lcms-link-page-list', 'value' : val, 'type' : 'text'}).setStyle({}).appendTo(contain);


						$.ajax({
							url: base_url + 'page/ajax/page_nav',
							success: function(html){
								$('#lcms-link-page-list').html(html);
								$('#lcms-link-page-list select').change(function(){
									$('#href').val('p/'+$(this).val());
								});
							}
						})
...
var nicEditorSelect = bkClass.extend({
	...
	open : function() {
		this.pane = new nicEditorPane(this.items,this.ne,{width : '88px', padding: '0px', borderTop : '1px solid #ccc', borderLeft : '1px solid #ccc', borderRight : '1px solid #ccc', borderBottom : '0px', backgroundColor : '#fff'});
...
var nicLinkButton = nicEditorAdvancedButton.extend({	
	addPane : function() {
		...
		this.addForm({
			...
			'page_list' : {type : 'page_list', txt : 'Pages'},
...
var nicUploadButton = nicEditorAdvancedButton.extend({	
	...
	addForm : function() {
		...
		if(this.uri == this.nicURI) {
			myDoc.write('<div style="position: fixed; margin-left: 160px;"><img src="http://imageshack.us/img/imageshack.png" width="30" style="float: left;" /><div style="float: left; margin-left: 5px; font-size: 10px;">Hosted by<br /><a href="http://www.imageshack.us/" target="_blank">ImageShack</a></div></div>');
		}
...
nicEditor = nicEditor.extend({
        floatingPanel : function() {
                this.floating = new bkElement('DIV').setStyle({position: 'fixed', top : '-1000px'}).appendTo(document.body);
...
var nicCodeButton = nicEditorAdvancedButton.extend({
	...
	addPane : function() {
		this.addForm({
			...
			'code' : {type : 'content', 'value' : style_html(this.ne.selectedInstance.getContent(),{'indent_size': 1,'indent_char': '\t'}), style : {width: '340px', height : '200px'}}
		});
	},
